...
1. Deploy version 2.0.0 while legacy snippets and UI definitions remain enabled.
...
The execute command saves option `hprwc_legacy_migration_manifest` before mutation. It disables the audited snippets, removes only the `publication` entry from CPT UI, and changes only the selected ACF taxonomy/group definitions from `publish` to `draft`.
